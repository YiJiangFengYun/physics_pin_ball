...
var World = /** @class */ (function (_super) {
     ...
     */
    World.prototype.step2 = function (dtPerIte, iterations) {
        if (!dtPerIte)
            return;
        if (!iterations)
            return;
        ...
        for (var iteration = 0; iteration < iterations; ++iteration) {
            this.emit(event_1.Event.PRE_FORWARD);
            var bulletCount = this.bulletCount;
            var bullets = this.bullets;
            for (var bulletIndex = 0; bulletIndex < bulletCount; ++bulletIndex) {
                ...
                var borderCount = borders.length;
                ...
                if (collided && collisionNormal.isZero() == false) {
                    ...
                    nextPos_1.x = bullet.pos.x + bullet.velocity.x * dtPerIte;
                    nextPos_1.y = bullet.pos.y + bullet.velocity.y * dtPerIte;
                    bullet.pos = nextPos_1;
                }
            }
            this.emit(event_1.Event.POST_FORWARD);
        }
    ...
    World.prototype._collideObj = function (bullet, object, collisionResult, collsionNormal) {
        ...
        if (!collisionResult.relected && preObjCollided == false && objCollided == true) {
            this.emit(event_1.Event.COLLISION_BEGIN, bullet, object);
            object.emit(event_1.Event.COLLISION_BEGIN, bullet);
            bullet.emit(event_1.Event.COLLISION_BEGIN, object);
        }
        if (objCollided) {
            this.emit(event_1.Event.COLLIDED, bullet, object);
            object.emit(event_1.Event.COLLIDED, bullet);
            bullet.emit(event_1.Event.COLLIDED, object);
        }
        if (!collisionResult.relected && preObjCollided == true && objCollided == false) {
            this.emit(event_1.Event.COLLISION_END, bullet, object);
            object.emit(event_1.Event.COLLISION_END, bullet);
            bullet.emit(event_1.Event.COLLISION_END, object);
        }
        return collided;
    };
    World.prototype._getBulletCoverItemSpaces = function (bullet, result) {
        ...
        var itemWidth = this.itemWidth;
        var itemHeight = this.itemHeight;
        var rowCount = this.itemRowCount - 1;
        var colCount = this.itemColCount - 1;
        var startCol = Math.floor((bounds.minX - startX) / itemWidth);
        var endCol = Math.ceil((bounds.maxX - startX) / itemWidth);
        var startRow = Math.floor((bounds.minY - startY) / itemHeight);
        var endRow = Math.ceil((bounds.maxY - startY) / itemHeight);
    ...
    World.vectorHelper = new vector_1.Vector();
    return World;
}(EventEmitter));
exports.World = World;
